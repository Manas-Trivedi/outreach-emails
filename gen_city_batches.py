...
CITY_RULES = [
    ("Bengaluru",          ("bengaluru", "bangalore", "bellandur", "koramangala",
                            "hsr layout", "indiranagar", "whitefield", "electronic city")),
...
for city in sorted(by_city, key=lambda c: (-len(by_city[c]), c)):
    ...
    for i in range(0, len(group), BATCH_SIZE):
        ...
        total += len(part)
        files += 1
    print(f"{city:22} {len(group):5} emails -> {(len(group) + BATCH_SIZE - 1) // BATCH_SIZE} file(s)")
